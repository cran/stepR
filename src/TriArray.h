...
template <class T>
TriArray<T>::TriArray() {
  Rcpp::stop("TriArray needs a size!");
}

template <class T>
TriArray<T>::TriArray(unsigned int n) {
  if(n <= 0) Rcpp::stop("TriArray needs a postive size!");
  size = n;
...
T& TriArray<T>::operator()(unsigned int i, unsigned int j) {
  if(i >= size) Rcpp::stop("First index out of bound!");
  if(j >= size) Rcpp::stop("Second index out of bound!");
  if(i > j) Rcpp::stop("First index may not exceed second index!");
...
T& TriArrayFF<T>::operator()(unsigned int i, unsigned int j) {
  if(i >= this->size) Rcpp::stop("First index out of bound!");
  if(j >= this->size) Rcpp::stop("Second index out of bound!");
  if(i > j) Rcpp::stop("First index may not exceed second index!");
