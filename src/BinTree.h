...
template <class T>
BinTree<T>::BinTree() {
  Rcpp::stop("BinTree needs to contain at least one node!");
}
...
void BinTree<T>::up() {
  if(s.size() == 1) {
    Rcpp::stop("There is no element above the root!");
  } else {
    s.pop();
  }
}
...
void BinTree<T>::left() {
  if(isLeaf()) {
    Rcpp::stop("There is no element below a leaf!");
  } else {
    s.push(s.top()->left);
  }
}
...
void BinTree<T>::addLeft(T value) {
  if(isLeaf()) {
    ...
    *m = Node(l, r, r->isRight); // new middle node
    ...
    Rcpp::stop("Cannot add element to non-leaf!");
  }
}
