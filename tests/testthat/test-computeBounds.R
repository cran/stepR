...
test_that("argument q is computed correctly", {
  ...
  expect_identical(computeBounds(testy, alpha = 0.014, stat = teststat,  nq = 100L,
                   ...
                   computeBounds(testy, intervalSystem = "all", lengths = 1L,
                           q = critVal(alpha = 0.014, intervalSystem = "all", lengths = 1L,
                                       stat = teststat, n = testn, nq = 100L)))
...
test_that("family 'jsmurf' works", {
  ...
  expect_error(computeBounds(family = "jsmurf", q = testq, filter = testfilter))
  expect_error(computeBounds(numeric(0), q = testq, filter = testfilter))
  expect_identical(computeBounds(testy, family = "jsmurf", q = testq, filter = testfilter),
                   computeBounds(testy, family = "jsmurf", intervalSystem = "dyaLen",
                                 lengths = 2^(4:6), q = testq, filter = testfilter))
  expect_error(computeBounds(as.integer(testy), family = "jsmurf", q = testq,
                             filter = testfilter))
  expect_error(computeBounds(c(testy, "s"), family = "jsmurf", q = testq,
                             filter = testfilter))
  expect_error(computeBounds(c(rnorm(10), NA), family = "jsmurf", q = testq,
                             filter = testfilter))

  bounds <- computeBounds(testy, family = "jsmurf", q = testq, filter = testfilter, sd = 1)
  testIndices(bounds = bounds, indices = indicesDyaLenLengths, n = testn, lengths = 2^(4:6))
  testBounds(bounds = bounds, singleBounds = singleBoundsJsmurf, y = testy, criticalValues = testq,
             filter = testfilter, sd = 1)
  testStat(bounds = bounds, singleStat = singleStatJsmurf, y = testy, criticalValues = testq,
           filter = testfilter, sd = 1)

  bounds <- computeBounds(testy, family = "jsmurfPS", q = testq, filter = testfilter, sd = 1)
  testIndices(bounds = bounds, indices = indicesDyaLenLengths, n = testn, lengths = 2^(4:6))
  testBounds(bounds = bounds, singleBounds = singleBoundsJsmurfPS, y = testy, criticalValues = testq,
             filter = testfilter, sd = 1)
  testStat(bounds = bounds, singleStat = singleStatJsmurfPS, y = testy, criticalValues = testq,
           filter = testfilter, sd = 1)

  bounds <- computeBounds(testy, family = "jsmurfLR", q = testq, filter = testfilter, sd = 1)
  testIndices(bounds = bounds, indices = indicesDyaLenLengths, n = testn, lengths = 2^(4:6))
  testBounds(bounds = bounds, singleBounds = singleBoundsJsmurfLR, y = testy, criticalValues = testq,
             filter = testfilter, sd = 1)
  testStat(bounds = bounds, singleStat = singleStatJsmurfLR, y = testy, criticalValues = testq,
           filter = testfilter, sd = 1)

  testn <- 23L
  testy <- c(rnorm(10, 1, 0.23), rnorm(testn - 10, -1, 0.34))
  testq <- 23:1 / 10
  testfilter <- lowpassFilter::lowpassFilter(type = "bessel", param = list(pole = 6L, cutoff = 0.2), sr = 1e4)

  expect_identical(computeBounds(testy, family = "jsmurf", q = testq, filter = testfilter),
                   computeBounds(testy, family = "jsmurf", q = testq, intervalSystem = "dyaLen",
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "jsmurfPS", q = testq, filter = testfilter),
                   computeBounds(testy, family = "jsmurfPS", q = testq, intervalSystem = "dyaLen",
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "jsmurfLR", q = testq, filter = testfilter),
                   computeBounds(testy, family = "jsmurfLR", q = testq, intervalSystem = "dyaLen",
                                 filter = testfilter))

  bounds <- computeBounds(testy, family = "jsmurf", q = testq, intervalSystem = "all",
                          filter = testfilter, sd = 1)
  testIndices(bounds = bounds, indices = indicesAllLengths, n = testn, lengths = (testfilter$len + 1):testn)
  testBounds(bounds = bounds, singleBounds = singleBoundsJsmurf, y = testy, criticalValues = testq,
             filter = testfilter, sd = 1)
  testStat(bounds = bounds, singleStat = singleStatJsmurf, y = testy, criticalValues = testq,
           filter = testfilter, sd = 1)
  bounds <- computeBounds(testy, family = "jsmurfPS", q = testq, intervalSystem = "all",
                          filter = testfilter, sd = 1)
  testIndices(bounds = bounds, indices = indicesAllLengths, n = testn, lengths = (testfilter$len + 1):testn)
  testBounds(bounds = bounds, singleBounds = singleBoundsJsmurfPS, y = testy, criticalValues = testq,
             filter = testfilter, sd = 1)
  testStat(bounds = bounds, singleStat = singleStatJsmurfPS, y = testy, criticalValues = testq,
           filter = testfilter, sd = 1)
  bounds <- computeBounds(testy, family = "jsmurfLR", q = testq, intervalSystem = "all",
                          filter = testfilter, sd = 1)
  testIndices(bounds = bounds, indices = indicesAllLengths, n = testn, lengths = (testfilter$len + 1):testn)
  testBounds(bounds = bounds, singleBounds = singleBoundsJsmurfLR, y = testy, criticalValues = testq,
             filter = testfilter, sd = 1)
  ...
  testn <- 37L
  testy <- c(rnorm(20, 1, 0.23), rnorm(testn - 20, -1, 0.34))
  testfilter <- lowpassFilter::lowpassFilter(type = "bessel", param = list(pole = 4L, cutoff = 0.0789), sr = 1e4)
  testq <- 37:1 / 10
  expect_identical(computeBounds(testy, family = "jsmurf", q = testq, filter = testfilter),
                   computeBounds(testy, family = "jsmurf", q = testq, lengths = 2^(4:5),
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "jsmurfPS", q = testq, filter = testfilter),
                   computeBounds(testy, family = "jsmurfPS", q = testq, lengths = 2^(4:5),
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "jsmurfLR", q = testq, filter = testfilter),
                   computeBounds(testy, family = "jsmurfLR", q = testq, lengths = 2^(4:5),
                                 filter = testfilter))

  testq <- 6:1
  bounds <- computeBounds(testy, family = "jsmurf", q = testq, intervalSystem = "all",
                          lengths = c(16:19, 35:36), filter = testfilter, sd = 1)
  testIndices(bounds = bounds, indices = indicesAllLengths, n = testn, lengths = c(16:19, 35:36))
  testBounds(bounds = bounds, singleBounds = singleBoundsJsmurf, y = testy, criticalValues = testq,
             lengths = c(16:19, 35:36), filter = testfilter, sd = 1)
  testStat(bounds = bounds, singleStat = singleStatJsmurf, y = testy, criticalValues = testq,
           lengths = c(16:19, 35:36), filter = testfilter, sd = 1)
...
test_that("argument q is tested and works for family 'jsmurf'", {
  testn <- 36L
  testy <- c(rnorm(12, 10, 0.13), rnorm(12, -10, 0.13), rnorm(12, 10, 0.13))
  testfilter <- lowpassFilter::lowpassFilter(type = "bessel", param = list(pole = 4L, cutoff = 0.1), sr = 1e4)
  teststat <- monteCarloSimulation(n = 36L, r = 100L, family = "jsmurf", filter = testfilter)

  expect_warning(ret <- computeBounds(testy, family = "jsmurf", filter = testfilter))
  expect_identical(ret, computeBounds(testy, family = "jsmurf", filter = testfilter, alpha = 0.5))
  expect_error(computeBounds(testy, q = rep(1, 7), alpha = 0.1, stat = teststat, family = "jsmurf",
                             filter = testfilter))
  expect_error(computeBounds(testy, q = rep(1, 4), alpha = 0.1, stat = teststat, family = "jsmurf",
                             filter = testfilter))

  expect_identical(computeBounds(testy, q = 3, family = "jsmurf", intervalSystem = "all",
                                 lengths = c(13:23, 25:27), penalty = "sqrt", filter = testfilter),
                   computeBounds(testy, family = "jsmurf", intervalSystem = "all", lengths = c(13:23, 25:27),
                                 filter = testfilter,
                                 q = critVal(q = 3, family = "jsmurf", intervalSystem = "all",
                                             lengths = c(13:23, 25:27), n = testn, penalty = "sqrt",
                                             filter = testfilter)))
  testq <- 1:45
  attr(testq, "n") <- 45L
  expect_identical(computeBounds(testy, family = "jsmurf", intervalSystem = "all", q = testq,
                                 filter = testfilter),
                   computeBounds(testy, family = "jsmurf", intervalSystem = "all", q = 1:36,
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "jsmurf", intervalSystem = "all", q = 1:36,
                                 lengths = 13:23, filter = testfilter),
                   computeBounds(testy, family = "jsmurf", intervalSystem = "all", q = 13:23,
                                 lengths = 13:23, filter = testfilter))

  ret <- computeBounds(testy, family = "jsmurf", alpha = 0.1, stat = teststat,
                       filter = testfilter)
  expect_identical(ret, computeBounds(testy, family = "jsmurf", alpha = 0.1, stat = teststat,
                                      nq = testn, intervalSystem = "dyaLen", lengths = 2^(4:5),
                                      penalty = "sqrt", filter = testfilter))
  expect_identical(ret, computeBounds(testy, family = "jsmurf", filter = testfilter,
                                      q = critVal(family = "jsmurf", alpha = 0.1, stat = teststat,
                                                  n = testn, filter = testfilter)))

  expect_error(computeBounds(testy, family = "jsmurf", alpha = "s", stat = teststat,
                             filter = testfilter))

  teststatmatrix <- monteCarloSimulation(n = 36L, r = 100L, family = "jsmurf",
                                         filter = testfilter, intervalSystem = "all")
  teststatvector <- monteCarloSimulation(n = 36L, r = 100L, family = "jsmurf",
                                         filter = testfilter, output = "maximum",
                                         intervalSystem = "all")
  expect_identical(computeBounds(testy, family = "jsmurf", filter = testfilter,
                                 intervalSystem = "all", penalty = "sqrt", alpha = 0.078, stat = teststatvector),
                   computeBounds(testy, family = "jsmurf", filter = testfilter,
                                 intervalSystem = "all",
                                 q = critVal(family = "jsmurf", filter = testfilter,
                                             intervalSystem = "all", alpha = 0.078, penalty = "sqrt",
                                             stat = teststatmatrix, n = testn)))

  teststatmatrix <- monteCarloSimulation(n = 36L, r = 100L, family = "jsmurfPS",
                                         filter = testfilter)
  teststatvector <- monteCarloSimulation(n = 36L, r = 100L, family = "jsmurfPS",
                                         filter = testfilter, output = "maximum",
                                         lengths = c(16, 32))
  ...
  teststat <- monteCarloSimulation(36L, r = 100L, family = "jsmurf",
                                   filter = testfilter)
  expect_identical(computeBounds(testy, alpha = 0.1, r = 100L, family = "jsmurf",
                                 filter = testfilter,
                                 options = list(simulation = "matrix", save = list(workspace = "matrix"),
                                                load = list(), envir = testStepR)),
                   computeBounds(testy, alpha = 0.1, stat = teststat, family = "jsmurf",
                                 filter = testfilter, options = list()))
  expect_identical(get("critValStepRTab", envir = testStepR, inherits = FALSE)$stat[[1]], teststat)
  expect_identical(length(get("critValStepRTab", envir = testStepR, inherits = FALSE)$stat), 1L)
  expect_identical(critVal(36L, alpha = 0.1, r = 50L, output = "vector", family = "jsmurf",
                           filter = testfilter,
                           options = list(simulation = "matrix", save = list(),
                                          load = list(workspace = "matrix"), envir = testStepR)),
                   critVal(36L, alpha = 0.1, stat = teststat, output = "vector", options = list(),
...
test_that("arguments in ... are tested and work for family jsmurf", {
  ...
  expect_error(computeBounds(testy, family = "jsmurf", q = 1, filter = testfilter, std = 1))
  expect_error(computeBounds(testy, family = "jsmurf", q = 1, intervalsystem = "all"))
  expect_error(computeBounds(testy, family = "jsmurfLR", q = 1))
  expect_error(computeBounds(testy, family = "jsmurfPS", q = 1, sd = 1))

  expect_error(computeBounds(testy, family = "jsmurfPS", q = 1, filter = unclass(testfilter)))

  expect_error(computeBounds(testy, sd = -0.1, family = "jsmurf", q = 1, filter = testfilter))

  expect_identical(computeBounds(testy, family = "jsmurfPS", q = 1, filter = testfilter),
                   computeBounds(testy, family = "jsmurfPS", q = 1, filter = testfilter,
...
test_that("family 'hjsmurf' works", {
  ...
  expect_error(computeBounds(family = "hjsmurf", q = testq, filter = testfilter))
  expect_error(computeBounds(numeric(0), q = testq, filter = testfilter))
  expect_identical(computeBounds(testy, family = "hjsmurf", q = testq, filter = testfilter),
                   computeBounds(testy, family = "hjsmurf", intervalSystem = "dyaLen",
                                 lengths = 2^(4:6), q = testq, filter = testfilter))
  expect_error(computeBounds(as.integer(testy), family = "hjsmurf", q = testq,
                             filter = testfilter))
  expect_error(computeBounds(c(testy, "s"), family = "hjsmurf", q = testq,
                             filter = testfilter))
  expect_error(computeBounds(c(rnorm(10), NA), family = "hjsmurf", q = testq,
                             filter = testfilter))

  bounds <- computeBounds(testy, family = "hjsmurf", q = testq, filter = testfilter)
  testIndices(bounds = bounds, indices = indicesDyaLenLengths, n = testn, lengths = 2^(4:6))
  testBounds(bounds = bounds, singleBounds = singleBoundsHjsmurf, y = testy, criticalValues = testq,
             filter = testfilter)
  testStat(bounds = bounds, singleStat = singleStatHjsmurf, y = testy, criticalValues = testq,
           filter = testfilter)

  bounds <- computeBounds(testy, family = "hjsmurfSPS", q = testq, filter = testfilter)
  testIndices(bounds = bounds, indices = indicesDyaLenLengths, n = testn, lengths = 2^(4:6))
  testBounds(bounds = bounds, singleBounds = singleBoundsHjsmurfSPS, y = testy, criticalValues = testq,
             filter = testfilter)
  testStat(bounds = bounds, singleStat = singleStatHjsmurfSPS, y = testy, criticalValues = testq,
           filter = testfilter)

  bounds <- computeBounds(testy, family = "hjsmurfLR", q = testq, filter = testfilter)
  testIndices(bounds = bounds, indices = indicesDyaLenLengths, n = testn, lengths = 2^(4:6))
  testBounds(bounds = bounds, singleBounds = singleBoundsHjsmurfLR, y = testy, criticalValues = testq,
             filter = testfilter)
  testStat(bounds = bounds, singleStat = singleStatHjsmurfLR, y = testy, criticalValues = testq,
           filter = testfilter)
  ...
  testy <- c(rnorm(10, 1, 0.23), rnorm(testn - 10, -1, 0.34))
  testq <- 23:1 / 10
  testfilter <- lowpassFilter::lowpassFilter(type = "bessel", param = list(pole = 6L, cutoff = 0.125), sr = 1e4)

  expect_identical(computeBounds(testy, family = "hjsmurf", q = testq, filter = testfilter),
                   computeBounds(testy, family = "hjsmurf", q = testq, intervalSystem = "dyaLen",
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "hjsmurfSPS", q = testq, filter = testfilter),
                   computeBounds(testy, family = "hjsmurfSPS", q = testq, intervalSystem = "dyaLen",
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "hjsmurfLR", q = testq, filter = testfilter),
                   computeBounds(testy, family = "hjsmurfLR", q = testq, intervalSystem = "dyaLen",
                                 filter = testfilter))

  bounds <- computeBounds(testy, family = "hjsmurf", q = testq, intervalSystem = "all",
                          filter = testfilter)
  testIndices(bounds = bounds, indices = indicesAllLengths, n = testn, lengths = (testfilter$len + 2):testn)
  testBounds(bounds = bounds, singleBounds = singleBoundsHjsmurf, y = testy, criticalValues = testq,
             filter = testfilter)
  testStat(bounds = bounds, singleStat = singleStatHjsmurf, y = testy, criticalValues = testq,
           filter = testfilter)
  bounds <- computeBounds(testy, family = "hjsmurfSPS", q = testq, intervalSystem = "all",
                          filter = testfilter)
  testIndices(bounds = bounds, indices = indicesAllLengths, n = testn, lengths = (testfilter$len + 2):testn)
  testBounds(bounds = bounds, singleBounds = singleBoundsHjsmurfSPS, y = testy, criticalValues = testq,
             filter = testfilter)
  ...
  testq <- 37:1 / 10
  expect_identical(computeBounds(testy, family = "hjsmurf", q = testq, filter = testfilter),
                   computeBounds(testy, family = "hjsmurf", q = testq, lengths = 2^(4:5),
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "hjsmurfSPS", q = testq, filter = testfilter),
                   computeBounds(testy, family = "hjsmurfSPS", q = testq, lengths = 2^(4:5),
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "hjsmurfLR", q = testq, filter = testfilter),
                   computeBounds(testy, family = "hjsmurfLR", q = testq, lengths = 2^(4:5),
                                 filter = testfilter))

  testq <- 6:1
  testfilter <- lowpassFilter::lowpassFilter(type = "bessel", param = list(pole = 4L, cutoff = 0.0789), sr = 1e4)
  bounds <- computeBounds(testy, family = "hjsmurf", q = testq, intervalSystem = "all",
                          lengths = c(16:19, 35:36), filter = testfilter)
  testIndices(bounds = bounds, indices = indicesAllLengths, n = testn, lengths = c(16:19, 35:36))
  testBounds(bounds = bounds, singleBounds = singleBoundsHjsmurf, y = testy, criticalValues = testq,
             lengths = c(16:19, 35:36), filter = testfilter)
  testStat(bounds = bounds, singleStat = singleStatHjsmurf, y = testy, criticalValues = testq,
...
test_that("argument q is tested and works for family 'hjsmurf'", {
  testn <- 36L
  testy <- c(rnorm(12, 10, 0.13), rnorm(12, -10, 0.13), rnorm(12, 10, 0.13))
  testfilter <- lowpassFilter::lowpassFilter(type = "bessel", param = list(pole = 4L, cutoff = 0.1), sr = 1e4)
  teststat <- monteCarloSimulation(n = 36L, r = 100L, family = "hjsmurf", filter = testfilter)

  expect_warning(ret <- computeBounds(testy, family = "hjsmurf", filter = testfilter))
  expect_identical(ret, computeBounds(testy, family = "hjsmurf", filter = testfilter, alpha = 0.5))
  expect_error(computeBounds(testy, q = rep(1, 7), alpha = 0.1, stat = teststat, family = "hjsmurf",
                             filter = testfilter))
  expect_error(computeBounds(testy, q = rep(1, 4), alpha = 0.1, stat = teststat, family = "hjsmurf",
                             filter = testfilter))

  expect_identical(computeBounds(testy, q = 3, family = "hjsmurf", intervalSystem = "all",
                                 lengths = c(13:23, 25:27), penalty = "sqrt", filter = testfilter),
                   computeBounds(testy, family = "hjsmurf", intervalSystem = "all", lengths = c(13:23, 25:27),
                                 filter = testfilter,
                                 q = critVal(q = 3, family = "hjsmurf", intervalSystem = "all",
                                             lengths = c(13:23, 25:27), n = testn, penalty = "sqrt",
                                             filter = testfilter)))
  testq <- 1:45
  attr(testq, "n") <- 45L
  expect_identical(computeBounds(testy, family = "hjsmurf", intervalSystem = "all", q = testq,
                                 filter = testfilter),
                   computeBounds(testy, family = "hjsmurf", intervalSystem = "all", q = 1:36,
                                 filter = testfilter))
  expect_identical(computeBounds(testy, family = "hjsmurf", intervalSystem = "all", q = 1:36,
                                 lengths = 13:23, filter = testfilter),
                   computeBounds(testy, family = "hjsmurf", intervalSystem = "all", q = 13:23,
                                 lengths = 13:23, filter = testfilter))

  ret <- computeBounds(testy, family = "hjsmurf", alpha = 0.1, stat = teststat,
                       filter = testfilter)
  expect_identical(ret, computeBounds(testy, family = "hjsmurf", alpha = 0.1, stat = teststat,
                                      nq = testn, intervalSystem = "dyaLen", lengths = 2^(4:5),
                                      penalty = "weights", filter = testfilter))
  expect_identical(ret, computeBounds(testy, family = "hjsmurf", filter = testfilter,
                                      q = critVal(family = "hjsmurf", alpha = 0.1, stat = teststat,
                                                  n = testn, filter = testfilter)))

  expect_error(computeBounds(testy, family = "hjsmurf", alpha = "s", stat = teststat,
                             filter = testfilter))

  teststatmatrix <- monteCarloSimulation(n = 36L, r = 100L, family = "hjsmurf",
                                         filter = testfilter, intervalSystem = "all")
  teststatvector <- monteCarloSimulation(n = 36L, r = 100L, family = "hjsmurf",
                                         filter = testfilter, output = "maximum",
                                         intervalSystem = "all", penalty = "sqrt")
  expect_identical(computeBounds(testy, family = "hjsmurf", filter = testfilter,
                                 intervalSystem = "all", penalty = "sqrt", alpha = 0.078, stat = teststatvector),
                   computeBounds(testy, family = "hjsmurf", filter = testfilter,
                                 intervalSystem = "all",
                                 q = critVal(family = "hjsmurf", filter = testfilter,
                                             intervalSystem = "all", alpha = 0.078, penalty = "sqrt",
                                             stat = teststatmatrix, n = testn)))

  teststatmatrix <- monteCarloSimulation(n = 36L, r = 100L, family = "hjsmurfSPS",
                                         filter = testfilter)
  teststatvector <- monteCarloSimulation(n = 36L, r = 100L, family = "hjsmurfSPS",
                                         filter = testfilter, output = "maximum",
                                         lengths = c(16, 32))

  expect_identical(computeBounds(testy, alpha = 0.1, r = 100L, lengths = 13:17, penalty = "log", nq = 100,
                                 family = "hjsmurfSPS", filter = testfilter, intervalSystem = "all",
                                 options = list(simulation = "vectorIncreased", save = list(), load = list())),
                   computeBounds(testy, lengths = 13:17, family = "hjsmurfSPS", filter = testfilter,
                                 intervalSystem = "all",
                                 q = critVal(n = 100, alpha = 0.1, r = 100L, lengths = 13:17, intervalSystem = "all",
                                             penalty = "log", family = "hjsmurfSPS", filter = testfilter,
                                             options = list(simulation = "vector", save = list(),
                                                            load = list()))))

  testStepR <- new.env()
  teststat <- monteCarloSimulation(36L, r = 100L, family = "hjsmurf",
                                   filter = testfilter)
  expect_identical(computeBounds(testy, alpha = 0.1, r = 100L, family = "hjsmurf",
                                 filter = testfilter,
                                 options = list(simulation = "matrix", save = list(workspace = "matrix"),
                                                load = list(), envir = testStepR)),
                   computeBounds(testy, alpha = 0.1, stat = teststat, family = "hjsmurf",
                                 filter = testfilter, options = list()))
  expect_identical(get("critValStepRTab", envir = testStepR, inherits = FALSE)$stat[[1]], teststat)
  expect_identical(length(get("critValStepRTab", envir = testStepR, inherits = FALSE)$stat), 1L)
  expect_identical(critVal(36L, alpha = 0.1, r = 50L, output = "vector", family = "hjsmurf",
                           filter = testfilter,
                           options = list(simulation = "matrix", save = list(),
                                          load = list(workspace = "matrix"), envir = testStepR)),
                   critVal(36L, alpha = 0.1, stat = teststat, output = "vector", options = list(),
                           family = "hjsmurf", filter = testfilter))
  remove(critValStepRTab, envir = testStepR)
})
...
test_that("arguments in ... are tested and work for family hjsmurf", {
  ...
  expect_error(computeBounds(testy, family = "hjsmurf", q = 1, filter = testfilter, sd = 1))
  expect_error(computeBounds(testy, family = "hjsmurf", q = 1, intervalsystem = "all"))
  expect_error(computeBounds(testy, family = "hjsmurfLR", q = 1))

  expect_error(computeBounds(testy, family = "hjsmurfSPS", q = 1, filter = unclass(testfilter)))
})

test_that("families LR and 2Param lead to errors", {
  testn <- 27L
  testy <- rnorm(testn)
  testfilter <- lowpassFilter::lowpassFilter(type = "bessel", param = list(pole = 4L, cutoff = 0.1), sr = 1e4)

  expect_error(computeBounds(testy, family = "LR", q = rep(1, testn), filter = testfilter))
  expect_error(computeBounds(testy, family = "2Param", q = rep(1, testn), filter = testfilter))
})
